    ...
    populate(view) {
        ...
        const edges = [
            makeLine(view, 'edge-start', [{ x: 170, y: 104 }, { x: 180, y: 150 }], {
                from: { node: 'start', handle: NodeHandle.S },
                to: { node: 'qualify', handle: NodeHandle.N },
            }),
            makeLine(view, 'edge-no', [{ x: 250, y: 200 }, { x: 360, y: 190 }], {
                from: { node: 'qualify', handle: NodeHandle.E },
                to: { node: 'nurture', handle: NodeHandle.W },
            }),
            makeLine(view, 'edge-yes', [{ x: 145, y: 250 }, { x: 140, y: 300 }], {
                from: { node: 'qualify', handle: NodeHandle.S },
                to: { node: 'demo', handle: NodeHandle.N },
            }),
            makeLine(view, 'edge-demo', [{ x: 240, y: 340 }, { x: 300, y: 340 }], {
                from: { node: 'demo', handle: NodeHandle.E },
                to: { node: 'close', handle: NodeHandle.W },
            }),
        ];
