...
export class UmlMethodAdapter extends RectangleAdapter {

    public static TYPE = 'uml_method';
    ...
    public override render(node: INode, context: CanvasRenderingContext2D, show?: 'all' | 'quick'): void {
        ...
        if (node.points.length > 1) {
            ...
            if (show !== 'quick') {
                ...
                const right = node.specific?.returns ? `: ${node.specific?.returns ?? ''}` : '';
    ...
    specificOptions(node: INode, path: string): SpecificOptions | undefined {
        if (path === 'returns' || path === 'specific.returns') {
            return {
                label: 'Returns',
                datatype: 'string',
            }
