...
/**
 * SpeechBubbleAdapter is a node adapter responsible for rendering speech bubble nodes in the diagram. 
 ...
 * Registers with the NodeRegistry under the name 'speech_bubble'.
 */
export class SpeechBubbleAdapter extends RectangleAdapter {

    public static TYPE = 'speech_bubble';
    ...
    public override render(node: INode, context: CanvasRenderingContext2D, show?: 'all' | 'quick'): void {
        ...
        if (node.points.length > 1) {
            ...
            let tail_width = Math.max(16 - radius, 0);
            let tail_height = 16;
            let tail_root: IPoint;
            let tail_tip: IPoint;
            ...
            switch (node.textStyle?.align) {
                case 'left':
                    ...
                    path.lineTo(tail_root.x + tail_width, rect.top + rect.height);
                    path.lineTo(tail_tip.x, tail_tip.y);
                    break;
                case 'right':
                    tail_root = {
                        x: to.x - radius,
                    ...
                    path.arcTo(rect.left + rect.width, rect.top + rect.height, tail_tip.x, tail_tip.y, radius);
                    path.lineTo(tail_tip.x, tail_tip.y);
                    path.lineTo(tail_root.x - tail_width, rect.top + rect.height);
                    break;
                case 'center':
                default:
                    tail_width = Math.max(tail_width, tail_height);
                    tail_width = Math.min(tail_width, rect.width - 2 * radius);
                    tail_root = {
                        x: (from.x + to.x) / 2,
                        y: to.y
                    };
                    ...
                    path.arcTo(rect.left + rect.width, rect.top + rect.height, rect.left + rect.width - radius, rect.top + rect.height, radius);
                    path.lineTo(tail_root.x + tail_width / 2, rect.top + rect.height);
                    path.lineTo(tail_root.x, tail_tip.y);
                    path.lineTo(tail_root.x - tail_width / 2, rect.top + rect.height);
            }
            // path.lineTo(tail_root.x - 8, rect.top + rect.height);
    ...
    //     // Ensure missing points are positioned properly
    //     const tail_root = {
    //         x: from.x + (to.x - from.x) / 4,
    //         y: to.y
    //     };
    //     // node.points[2] = (tail_root);
    //     const tail_tip = {
    //         x: tail_root.x,
    //         y: tail_root.y + 8
    //     };
    ...
    //     context.beginPath();
    //     context.arc(tail_root.x, tail_root.y, DiagramConstants.HANDLE_HIT_EPSILON, 0, 2 * Math.PI);
    //     context.arc(tail_tip.x, tail_tip.y, DiagramConstants.HANDLE_HIT_EPSILON, 0, 2 * Math.PI);
    //     context.fill();
    //     context.stroke();
    ...
    public geometryOptions(node: INode, path: string): SpecificOptions | undefined {
        ...
        // if (path === 'geometry.align' || path === 'align') {
        //     return {
        //         label: 'Tail Position',
        //         datatype: 'number',
        //     }
